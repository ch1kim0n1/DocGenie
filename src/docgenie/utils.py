...
def load_gitignore_spec(root_path: Path) -> PathSpec | None:
    """Load .gitignore rules as a pathspec matcher."""
...
def is_path_ignored_by_gitignore(
    rel_path: str, matcher: PathSpec | None, *, is_dir: bool = False
) -> bool:
    """Check whether a relative path matches .gitignore rules."""
    if matcher is None:
        return False
    candidate = rel_path.rstrip("/")
    if is_dir:
        candidate = f"{candidate}/"
    return matcher.match_file(candidate)


def is_hidden_path(path: str) -> bool:
    """Return True when any segment of path starts with '.'."""
    normalized = path.replace("\\", "/")
    parts = [segment for segment in normalized.split("/") if segment not in {"", "."}]
    return any(part.startswith(".") for part in parts)


def is_probably_generated_file(file_path: str, extra_patterns: List[str] | None = None) -> bool:
    """Best-effort heuristic for autogenerated or low-signal files."""
    path_str = str(file_path)
    patterns = GENERATED_FILE_PATTERNS.copy()
    if extra_patterns:
        patterns.extend(extra_patterns)

    lowered = path_str.lower()
    noisy_dirs = ["dist/", "build/", "coverage/", ".next/", ".nuxt/", "target/", "__pycache__/"]
    if any(segment in lowered.replace("\\", "/") for segment in noisy_dirs):
        return True
...
def detect_packages(root_path: Path) -> list[dict[str, Any]]:
    """Detect package/service boundaries for monorepo-style docs."""
    packages: list[dict[str, Any]] = []
    root = root_path.resolve()
    for dirpath, _dirs, files in os.walk(root):
        manifest = next((f for f in files if f in PACKAGE_MANIFESTS), None)
        if not manifest:
            continue
        pkg_path = Path(dirpath)
        rel = pkg_path.relative_to(root).as_posix()
        parent_path: str | None = None
        for existing in sorted(packages, key=lambda p: len(str(p["path"]))):
            existing_path = str(existing["path"])
            if rel != existing_path and rel.startswith(f"{existing_path}/"):
                parent_path = existing_path
        packages.append(
            {
                "path": rel if rel != "." else ".",
                "package_type": PACKAGE_MANIFESTS[manifest],
                "manifest": manifest,
